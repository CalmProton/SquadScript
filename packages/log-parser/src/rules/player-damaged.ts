...
export const playerDamagedRule = defineRule<PlayerDamagedEvent>({
  ...
  parse(match, context) {
    ...
    if (!timestamp || !chainIDStr || !victimName || !damageStr || !attackerName || !idsString || !attackerController || !weapon) {
      context.logger.warn('player-damaged', 'Missing required fields in player damaged event');
      return null;
    }
    ...
    // Store session data for wound/death correlation - use fallback for type safety
    ...
    const attacker: Player = {
      ...
      name: storedAttacker?.name ?? attackerName ?? 'Unknown',
      teamID: null,
      squadID: null,
      isSquadLeader: false,
      role: null,
