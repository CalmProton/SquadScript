...
RCON_CONFIG="${STEAMAPPDIR}/SquadGame/ServerConfig/Rcon.cfg"
RCON_PASSWORD_VALUE="${RCONPASSWORD:-${RCON_PASSWORD:-}}"
...
sed -i -e 's/Port=21114/'"Port=${RCONPORT}"'/g' "${RCON_CONFIG}"

if [[ -n "${RCON_PASSWORD_VALUE}" ]]; then
	if grep -q '^Password=' "${RCON_CONFIG}"; then
		sed -i -e 's/^Password=.*/'"Password=${RCON_PASSWORD_VALUE}"'/' "${RCON_CONFIG}"
	else
		printf '\nPassword=%s\n' "${RCON_PASSWORD_VALUE}" >> "${RCON_CONFIG}"
	fi
else
	echo "WARNING: RCON password is empty (set RCONPASSWORD or RCON_PASSWORD)"
fi
